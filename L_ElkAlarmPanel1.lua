...
local function processTaskReport (data)
  ...
  if (g_tasks[taskNo] ~=nil) then
    ...
    luup.call_delay("toggleTask", 15, taskDevNo)
  else
    debug("processTaskReport: No device on Vera to present Task.")
  end
end

local function toggleTask(taskDevNo)
    luup.variable_set(SWP_SID, "Status", "0",  taskDevNo)
end
...
local function createUsers()
  ...
  while (i >=1 and i <=199) do
    ...
    if (status) then
      ...
      local labelId = data:sub(1, 2)
      local user = data:sub(6)
      debug("createUsers: Creating user ".. i ..".")

      g_users[i] = {}
      debug(string.format("createUsers: Adding name %s to user %03i.", user, i))
      g_users[i].label = user
    end
    i=i+1
  end
...
local function createTasks()
  ...
  while (i >=1 and i <=32) do
    ...
    if (status) then
      ...
      data = data:gsub("(%s+)$", "")
      local labelId = data:sub(1, 2)
      local task = tonumber(data:sub(3,5))
      if task == 0 then
        debug(string.format("createTasks: Skipping task %03i.", i))
        break
      else
        g_tasks[task] = {}
        local label = (data:sub(6) or "")
        debug(string.format("createTasks: Adding name %s to task %03i.", label, task))
        g_tasks[task].label = label
        i=task
      end
    end
    i=i+1
...
local function createOutputs()
  ...
  while (i >=1 and i <=64) do
    ...
    if (status) then
      ...
      data = data:gsub("(%s+)$", "")
      local labelId = data:sub(1, 2)
      local output = tonumber(data:sub(3,5))
      if output == 0 then
       debug(string.format("createOutputs: Skipping output %03i.", i))
       break
      else
        g_outputs[output] = {}
        local label = (data:sub(6) or "")
        debug(string.format("createOutputs: Adding name %s to output %03i.", label, output))
        g_outputs[output].label = label
        i=output
      end
    end
    i=i+1
...
local function createKeypads()
  ...
  while (i >=1 and i <=16) do
    ...
    if (status) then
      ...
      data = data:gsub("(%s+)$", "")
      local labelId = data:sub(1, 2)
      local keypad = tonumber(data:sub(3,5))
      if keypad == 0 then
...
local function createLights()
  ...
  while (i >=1 and i <=256) do
    ...
    if (status) then
      ...
      if index == 0 then
        debug(string.format("createOutputs: Skipping light %03i.", i))
        break
      else
        g_lights[index] = {}
        local label = (data:sub(6) or "")
...
function elkStartup (lul_device)
  ...
  local plcControl = luup.variable_get(ELK_SID, "PLCControl", elk_device) or ""
  if (plcControl == "") then
    luup.variable_set(ELK_SID, "PLCControl", "1", elk_device)
    log("(ElkPlugin::elkStartup) PLC lighting control set to True/Yes (default.")
    appendLights(rootPtr)
  elseif (plcControl == "0") then
    log("(ElkPlugin::elkStartup) PLC lighting control set to false/No.")
  end
